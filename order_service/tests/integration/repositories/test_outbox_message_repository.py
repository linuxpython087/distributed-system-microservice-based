...
def test_mark_message_publised(session):
    ...
    published = repo.get_outbox_message(
        outbox.id
    )
...
def test_claim_pending_messages(session):
    ...
    integration_event = (
        OrderIntegrationEventFactory.from_order(order)
    )

    outbox = OutboxMessage.from_integration_event(
        integration_event,
        order.id
    )
    ...
    claimed = repo.claim_pending_messages(
        limit=10,
        worker_id="worker-1"
    )
...
def test_claim_respects_limit(session):
    ...
    for _ in range(5):
        ...
        repo.add(
            OutboxMessage.from_integration_event(
                event,
                order.id
            )
        )
...
def test_message_with_5_retries_is_not_retryable(
    session
):
    repo = SqlalchemyOutboxMessageRepository(session)
